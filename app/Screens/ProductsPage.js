...
const ProductsPage = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const fetchCartItems = async () => {
      const user = getCurrentUser(); // Fonction getCurrentUser à implémenter
    ...
    const fetchProducts = async () => {
      try {
        ...
        // Extraire les données des produits depuis le snapshot
        const productsData = snapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        }));

        // Mettre à jour l'état des produits avec les données récupérées
